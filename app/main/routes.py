...
def index():
    # Plot 1
    obj_data1 = session.execute("SELECT date, v10, v11 FROM emsdata ORDER BY date DESC").fetchall()
    ...
    # Plot 2
    obj_data2 = session.execute("SELECT date, v12 FROM emsdata ORDER BY date DESC").fetchall()
    data_v12 = [[int(item[0].timestamp()) * 1000, item[1]] for item in obj_data2]

    # Plot 3
    obj_data3 = session.execute("SELECT date, v13 FROM emsdata ORDER BY date DESC").fetchall()
    data_v13 = [[int(item[0].timestamp()) * 1000, item[1]] for item in obj_data3]

    # Plot 4
    obj_data4 = session.execute("SELECT daytime, billVpp1, billVpp2, billVpp3 FROM electrbill ORDER BY daytime DESC").fetchall()
